...
public class TestCompetitor7 extends Competitor {
	...
	public TestCompetitor7(String thisName) {
		super(thisName);
	...
	public void doManuever() {
		...
		if (listInRangeNugget.size() > 0) {
			...
			if (((energyLevel - ENERGY_NORMAL_RESERVE) / 2) > newDistance) {
				// move the smaller of the two
				distanceToActuallyMove = newDistance;
			}
			else {
				distanceToActuallyMove = ((energyLevel - ENERGY_NORMAL_RESERVE) / 2);
			}


			legalMove(distanceToActuallyMove);                            // If we have enough energy, legalMove()
